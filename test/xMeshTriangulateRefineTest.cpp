...
void refineWithPointCreator(xMesh mesh) {
    mesh.refine(mesh.getPolygon(17), new CentroidPointCreator);
    mesh.printInFile("refinedWithCreatorPoly.txt");
}

void refineWithPointGenerator(xMesh mesh) {
    mesh.refine(mesh.getPolygon(17), PointGenerator(functions::constantAlternating(), functions::constant()), 3, 3);
    mesh.printInFile("refinedWithGeneratorPoly.txt");
}
...
int main(){
    ...
    refineWithPointCreator(destructable);
